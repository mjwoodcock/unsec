...
#define calccrc(a) 
#define RUNMARK (unsigned char)0x90
    ...
    unsigned char byte;
{
    ...
    if (running) {
	if (!byte) {	/* means write RUNMARK to output */
	    calccrc(RUNMARK);
	    if (!testing)
...
Status
uncompress(complen, origlen, ifp, ofp, type)
